...
struct Vec2
{
    ...
    constexpr Vec2( Float theX, Float theY )
        : x( theX )
        , y( theY )
    {}
...
inline constexpr Vec2 operator*( Float s, Vec2 v )
{
    return v * s;
}
...
inline constexpr Float dot( Vec2 a, Vec2 b )
{
    return a.x * b.x + a.y * b.y;
}
...
inline constexpr Float cross( Vec2 a, Vec2 b )
{
    return a.x * b.y - a.y * b.x;
}

// cross( s, v ) and cross( v, s ) return perpendicular vectors
inline constexpr Vec2 cross( Float s, Vec2 v )
{
    return { -s * v.y, s * v.x };
}

inline constexpr Vec2 cross( Vec2 v, Float s )
{
    return { s * v.y, -s * v.x };
}

inline constexpr Vec2 perp( Vec2 v )
{
    return { -v.y, v.x };
}
...
struct Transform2D
{
    ...
    Mat3 toMatrix() const
    {
        return Mat3().translate( translation ).rotate( rotationRadians ).scale( scale );
    }
};
